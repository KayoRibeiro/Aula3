...
int quantidadeDeNosNaArvore(No* raiz){
	
 if(raiz == NULL)
        return 0;
   else
        return 1 + quantidadeDeNosNaArvore(raiz->ae) + quantidadeDeNosNaArvore(raiz->ad);
}
  	...
  	while(1) {
	    ...
	    switch(op) {
	    	case 1 : system("cls");
       		...
       		case 5 : system("cls"); 
					total =  quantidadeDeNosNaArvore(raiz);
			   		printf("\n Quantidade de Nos é %i", total);
       			   	system("pause>>null");
                  break;                               
